# -*- coding: utf-8 -*-
"""
Created on Thu Aug 22 11:56:39 2024
...
def create_model(input_shape, num_classes=1, use_pretrained=USE_PRETRAINED, trainable_layers=10):
    if use_pretrained:
        ...
        model = Sequential()
        model.add(Input(shape=input_shape)) # Input layer for shape inference
        ...
        dense_input_size = model.output_shape[1] # Access output shape after adding Flatten
...
def classify_patches(model, image, patch_size=PATCH_SIZE):
    ...
    for y in range(0, height - patch_size[1] + 1, patch_size[1]):
        for x in range(0, width - patch_size[0] + 1, patch_size[0]):
            ...
            # Ensure patch has correct number of channels for the model
            if patch.shape[-1] != model.input_shape[-1]:
