# -*- coding: utf-8 -*-
"""
Created on Sat Aug 24 12:59:25 2024
...
def load_data(images_dir, labels_path, patch_size=PATCH_SIZE, use_pretrained=USE_PRETRAINED):
    image_files = [
        f for f in os.listdir(images_dir) if f.lower().endswith(('.jpg', '.png', '.jpeg', '.tif', '.tiff'))
    ]
    images = []
    labels = []

    # Read GeoJSON file once before the loop
    gdf = gpd.read_file(labels_path)
    polygons = gdf.geometry.to_list()  # Extract polygons from GeoDataFrame
            ...
            for y in range(0, src.height - patch_size[1] + 1, patch_size[1]):
                for x in range(0, src.width - patch_size[0] + 1, patch_size[0]):
                    ...
                    if image.shape[-1] == 4:
                     ...
                     # Convert to 3 channels if using pretrained model and image is grayscale
                    ...
                    # Check if center of the patch is within any polygon
    ...
    # Convert the 'images' array to a regular NumPy array with a consistent shape
    consistent_shape = (len(images), IMG_HEIGHT, IMG_WIDTH, 3)  # Assuming 3 channels
    images = np.array([cv2.resize(img, (IMG_WIDTH, IMG_HEIGHT)) for img in images]).reshape(consistent_shape)

    return images, labels


# --- Model Creation (with More Unfrozen Layers) ---
...
def classify_patches(model, image, patch_size=PATCH_SIZE, use_pretrained=USE_PRETRAINED):
    ...
    for y in range(0, height - patch_size[1] + 1, patch_size[1]):
        for x in range(0, width - patch_size[0] + 1, patch_size[0]):
            ...
            if use_pretrained and patch.shape[-1] == 1: 
            ...
            print(f"Prediction for patch at ({x}, {y}): {prediction[0][0]}")  
