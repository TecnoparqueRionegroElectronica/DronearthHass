# -*- coding: utf-8 -*-
"""
Created on Tue Aug 20 09:28:48 2024
...
# --- Model Creation ---
def create_model(input_shape, num_classes=1, use_pretrained=True, trainable_layers=10):
    if use_pretrained:
        ...
        model = Sequential()
        model.add(Input(shape=input_shape)) # Define input shape for Sequential model
    ...
    return model
# Function to classify patches of an image
...
def load_data(images_dir, labels_path, img_height=IMG_HEIGHT, img_width=IMG_WIDTH):
    ...
    for image_file in image_files:
        ...
        with rasterio.open(image_path) as src:
            # Sample/Resize the image directly with rasterio
            image = src.read(
                out_shape=(src.count, img_height, img_width),
                resampling=rasterio.enums.Resampling.bilinear
            )
            image = np.moveaxis(image, 0, -1) # Reorder to (height, width, channels)
            image = image.astype(np.float32) # Use smaller float precision

            # Repeat grayscale image to create 3 channels if using ResNet
...
if __name__ == "__main__":
    ...
    images, labels = load_data(DATA_PATH, LABELS_PATH, img_height=IMG_HEIGHT, img_width=IMG_WIDTH)
    input_shape = images[0].shape
    
    model = create_model(input_shape, use_pretrained=False, trainable_layers=5)
